"""
Test script for heartbeat endpoint functionality.
This tests the POST /api/heartbeats/{monitor_id} endpoint.
"""
import asyncio
import sys
...
async def test_heartbeat_endpoint():
    """Test the heartbeat reception logic"""
    ...
    async with AsyncSessionLocal() as db:
        ...
        test_user = User(
            email=f"test_heartbeat_{uuid4()}@example.com"
        )
        db.add(test_user)
        await db.commit()
        ...
        # Step 3: Simulate receiving a heartbeat
        print("\n3. Simulating heartbeat reception...")
        current_time = datetime.now(timezone.utc)
        
        # Update monitor state (this is what the endpoint does)
        ...
        print("   Testing non-existent monitor...")
        fake_id = uuid4()
        result = await db.execute(
            select(Monitor).where(Monitor.id == fake_id)
        )
        fake_monitor = result.scalar_one_or_none()
        assert fake_monitor is None, "Should return None for non-existent monitor"
        print("   ✓ Non-existent monitor returns None (404 expected)")
        
        # Test with non-heartbeat monitor
        print("   Testing non-heartbeat monitor...")
        ...
        db.add(simple_monitor)
        await db.commit()
        await db.refresh(simple_monitor)
        
        assert simple_monitor.monitor_type != "heartbeat", "Should be simple type"
        print(f"   ✓ Simple monitor type: {simple_monitor.monitor_type} (400 expected)")
        
        # Cleanup
        print("\n7. Cleaning up test data...")
        await db.delete(heartbeat_record)
        await db.delete(monitor)
        await db.delete(simple_monitor)
...
if __name__ == "__main__":
    asyncio.run(test_heartbeat_endpoint())
